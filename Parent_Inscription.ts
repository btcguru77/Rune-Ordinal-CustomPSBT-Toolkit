...
const receiveAddress: string = "tb1pn4gr2asu3hynl9f5cd7pgstx2m9zstes72ztxtqmhtzvrvg7pjrqndpfgq";
const metadata = {
  'type': 'Bitmap',
...
// Temp txid
const tempTxid = "6c1368786c7576b2c338cd643964333cee08814170f40e4fe2c8696575907130";
const tempVout = 100;
...
const inscriptions = [
  {
    mimeType: "text/html",
    content: Buffer.from("Example metadata 1", "utf8"),
    filename: "first parent inscription"
  }
];

export function createMultiParentInscriptionTapScript(): Array<Buffer> {
  const keyPair = wallet.ecPair;
  const parentOrdinalStacks: any = [
    toXOnly(keyPair.publicKey),
    opcodes.OP_CHECKSIG,
  ...
  inscriptions.forEach((inscription, index) => {
    parentOrdinalStacks.push(
      ...
      1,
      1,
      Buffer.concat([Buffer.from(inscription.mimeType, "utf8")]),
      1,
      2,
      Buffer.from((546 * index).toString(16).padStart(4, '0'), 'hex').reverse(),
      1,
      5,
      inscription.content,
      opcodes.OP_0,
...
async function parentInscribe() {
  const keyPair = wallet.ecPair;
  const parentOrdinalStack = createMultiParentInscriptionTapScript();
